...
def main():
	...
	for i in range(0,lim):
		...
		nomfichier= "test3.8."+str(i)+".txt" #input("Nommer votre fichier de sortie : ")+".txt" # on nomme le fichier que l'on va créer à partir
...
def affectationPNN(dictTw,weights,x):#  affectationPNN affecte une valeur à chaque tweet selon les poids calculés
	...
	for i in range(len(dictTw)): # pour chaque tweet
		firstNum, secondNum, value, tweet, vector, vector2,twoValues=dictTw[i]# on calcule
		v=vector
		if x>0:
			v=vector2
		...
		dictTw[i]=(firstNum, secondNum, value, tweet, vector, vector2,twoValues)
	return dictTw

def actifLearnig(dictTw):
	trainingPlus=[]
	classify={}
	noClassify={}
	...
	for key in dictTw.keys(): # pour chaque tweet
		...
		if twoValues[0]==twoValues[1]:
			value= twoValues[0]
			values[value]=1
			trainingPlus.append((vector,vector2,values))
			classify[key]=firstNum, secondNum, value, tweet
		else:
			noClassify[key]=firstNum, secondNum, value, tweet, vector, vector2,twoValues
			opiff= random.randint(0, 1)
			value=twoValues[opiff]
		dictTw[key]=firstNum, secondNum, value, tweet
