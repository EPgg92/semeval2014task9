...
def main():
	...
	for i in range(0,lim):
		file= "training.txt"#input("Nom du fichier à traiter : ") # On récupére le nom du fichier à traité
		file= "development.input.txt"#input("Nom du fichier à traiter : ") # On récupére le nom du fichier à traité
		nomfichier= "test3.5."+str(i)+".txt" #input("Nommer votre fichier de sortie : ")+".txt" # on nomme le fichier que l'on va créer à partir
		ressources=fonctionRessources(files)
		train="training.txt"# notre fichier d'entraînement
		vectors, dictonary = tools(train,ressources) # vectors représente les vecteurs d'entraînement généré des tweets
		del train # dictonary est l'ensemble clé->mot minuscule : valeur-> indice pour tableau vectoriel
		data=readFile(file, dictonary,ressources) # data est l'ensemble des tweets à traité avec leurs vecteurs crée selon les mots du dictonnaire contnu
...
def fonctionRessources(files):
	dictUnigrams=readPmiFile(files[0])
	dictBigrams=readPmiFile(files[1])
	hashtag=readPmiFile(files[2])
	listSmiley=open(files[3],'r').readlines()
	for i in range(len(listSmiley)):
	    listSmiley[i]=set(listSmiley[i].split())
	setHashtagPos=set()
	setHashtagNeg=set()
	...
	ressources=listSmiley,setUniPos,setUniNeg,setBiPos,setBiNeg,setHashtagPos,setHashtagNeg,dictUnigrams,dictBigrams
	return ressources
...
def dictPmi(dictpmi):
	pos=set()
	neg=set()
	for key in dictpmi.keys():
		if float(dictpmi[key])>0:
			pos.add(key)
		else:
			neg.add(key)
	return pos,neg

def tools(train,ressources): # crée les outils nécessire à l'analyse à partir des données d'entrainment
	...
	for elts in tweets: # pour chauqe tweet:
		...
		vectorLing=linguisticVector(tweet, ressources)
		vector2=np.zeros(len(vectorLing))

		for j in range(len(vectorLing)):
			vector2=vectorLing
		#print((vector2))
...
def linguisticVector(tweet, ressources):
	listSmiley,setUniPos,setUniNeg,setBiPos,setBiNeg,setHashtagPos,setHashtagNeg,dictUnigrams,dictBigrams = ressources
	neutrality=0
	vector2=[]
	# on regarde si le dernier mot est positif ou négatif
	lastWord=tweet[len(tweet)-1]
	lastWordNeg=0
	lastWordPos=0
	if lastWord in dictUnigrams:
		if float(dictUnigrams[lastWord])>0:
			lastWordPos=1
		else:
			lastWordNeg=1
	else:
		neutrality+=1
	vector2.extend([lastWordPos,lastWordNeg])
	setTweet=set(tweet)

	# on crée les bigrams dans l'ensemble des mot du tweet.
	i=0
	for bi in tweet:
		if i+1 < len(tweet):
			setTweet.add(bi+" "+tweet[i+1])
		i+=1
	# on teste si il y a des smileys selon leur type.
	smiley=[]
	for smi in listSmiley:
		if not len(setTweet.intersection(smi))==0:
			smiley.append(1)
		else:
			smiley.append(0)
	vector2.extend(smiley) # et on les ajoute leur vectorification au vecteur à retouner
	if np.sum(smiley)==0:
		neutrality+=1

	hashtagPos, nbHashtagPos , hashtagNeg, nbHashtagNeg=[0]*4
	if not len(setTweet.intersection())==0:
		hashtagPos=1
		nbHashtagPos= len(setTweet.intersection(setHashtagPos))
	if not len(setTweet.intersection(setHashtagNeg))==0:
		hashtagNeg=1
		nbHashtagNeg= len(setTweet.intersection(setHashtagNeg))

	majoriteHashtagPos=0
	majoriteHashtagNeg=0
	if nbHashtagPos+nbHashtagNeg!=0:
		rapportHastagPos= nbHashtagPos/(nbHashtagPos+nbHashtagNeg)
		rapportHastagNeg= nbHashtagNeg/(nbHashtagPos+nbHashtagNeg)
		if rapportHastagPos>rapportHastagNeg:
			majoriteHashtagPos=1
		else:# rapportHastagPos<rapportHastagNeg:
			majoriteHashtagNeg=1
	else:
		neutrality+=1
	traitHastag=[hashtagPos,hashtagNeg,majoriteHashtagPos,majoriteHashtagNeg]
	vector2.extend(traitHastag)
	if np.sum(traitHastag)==0:
		neutrality+=1

	score=0
	score, traitUni = pmiGrams(score,setUniPos,setUniNeg,dictUnigrams,setTweet)
	vector2.extend(traitUni)
	if np.sum(traitUni)==0:
		neutrality+=1
	score, traitBi = pmiGrams(score,setBiPos,setBiNeg,dictBigrams,setTweet)
	vector2.extend(traitBi)
	if np.sum(traitBi)==0:
		neutrality+=1

	if score==0:
		neutrality+=1
	vector2.append(neutrality)
	return vector2

def pmiGrams(score, setPos, setNeg, dictGrams, setTweet):
	nbPos, nbNeg, pos, neg, rapportPos, rapportNeg, majoritePos, majoriteNeg=[0,0,0,0,0,0,0,0]
	if not len(setTweet.intersection(setPos))==0:
		pos=1
		nbPos= len(setTweet.intersection(setPos))
		for key in setTweet.intersection(setPos):
			score+=float(dictGrams[key])
	if not len(setTweet.intersection(setNeg))==0:
		neg=1
		nbBiNeg= len(setTweet.intersection(setNeg))
		for key in setTweet.intersection(setNeg):
			score+=float(dictGrams[key])
	if nbPos+nbNeg!=0:
		rapportPos=nbPos/(nbPos+nbNeg)
		rapportNeg=nbNeg/(nbPos+nbNeg)
		if rapportPos>rapportNeg:
			majoritePos=1
		elif rapportPos<rapportNeg:
			majoriteNeg=1
	trait=[pos,neg,majoritePos,majoriteNeg]
	return score, trait

def readFile(file, dictonary, ressources): # readFile(file, dictonary) lit le fichier à testé et crée les vecteurs des tweets grace au dictionnaire
	dictTw={}
	...
	for line in openFile: # pour chaque tweets à testé
		...
		vector2=linguisticVector(tweet, ressources)
...
def weightsValue(vectors):# weightsValue calcule les différents vecteurs de poids avec une tolérance d'erreur
	...
	newAllWeight=np.random.random_sample((len(vectors[0][1]),))
	print("Calcul des poids pour les neutres:")	# poids aléatoire utiliser pour les trois valeurs
	print(len(newAllWeight))
	wNeutral=neurone(vectors,newAllWeight, "neutral","ling", 0.50)
	print("Calcul des poids pour les negatifs:")
	wNegative=neurone(vectors,newAllWeight, "negative","ling", 0.25)
	print("Calcul des poids pour les positifs:")
	wPositive=neurone(vectors,newAllWeight, "positive","ling", 0.49)
...
def neurone(vectors,weight, SelecValue,traitement,pourcentErreur):
	...
	while True :# on commence une boucle perpétuelle
		...
		#print(str(erreur),str(len(vectors)),str(int(erreur/len(vectors)*100)) +"->"+ str(pourcentErreur*100)+"%" )
		if  erreur<pourcentErreur*len(vectors): # que l'on a pas un taux d'erreur satisfaisant
			break
