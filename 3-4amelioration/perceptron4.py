...
def main():
    ...
    vecteursE, dictionnaireModele1, dictionnaireModele2Local=vecteursEntrainement(tweetsEntrainement)
    tweetsAClassifier=vecteursClassication(tweetsAClassifier,dictionnaireModele1,dictionnaireModele2Local)
...
def lireFichierTweets(file, doublon):
    ...
    for line in file:
        ...
        tweet=line.split()
        cle = (tweet[0],tweet[1])# on crée la clé du dictionnaire avec les deux élément unique du tweets
        value= tweet[2]
        tweet=tweet[3:]
        ...
        if ((cle in dicoTweets and value!=dicoTweets[cle][0])or cle in elimine) and doublon:
            del dicoTweets[cle] # Si le tweet est déjà de dans avec une autre valeur alors on enléve toutes ses occurences
            elimine.add(cle)
        else:
            dicoTweets[cle]=(value,tweet,setTweet,vector,vector2,valeurPossibles)
    return dicoTweets
    ...
    for key in tweetsEntrainement.keys(): # pour chauqe tweet:
        ...
        for word in tweet: # on regarde si chaque mot est
            if word in dictionnaire: # dans le dictionnaire
                vector[dictionnaire[word]]=ponderation[value] # si oui on modifie le vecteur à l'indice du mot
        vector2 = np.zeros(len(dictionnaireModele2Local))
        for key in setTweet.intersection(set(dictionnaireModele2Local.keys())):
            vector2[dictionnaireModele2Local[key][0]]=dictionnaireModele2Local[key][0]*ponderation[value]
...
def vecteursClassication(tweetsAClassifier, dictionnaireEntrainement,dictionnaireModele2Local):
	for key in tweetsAClassifier.keys()  : # pour chaque tweets à testé
		...
		vector=np.zeros(len(dictionnaireEntrainement)) # on initialise le vecteur avec des zéros
		vector2 = np.zeros(len(dictionnaireModele2Local))
		for mot in tweet: # et pour chaque mot du tweet
			mot, motOK =verifierMot(mot) # selon cette expression réguliére
			if motOK and mot in dictionnaireEntrainement: # on regarde si ces mots en minuscule se trouve dans le dictionnaire
				vector[dictionnaireEntrainement[mot]]=1# si ou on change la valeur à l'indice du mot du dico dans le vecteur de ce tweet
...
def verifierMot(mot):
	...
	wordOK=re.search( r'[,!?.@#]*([A-Za-z]*)[,!?.]*', mot) # selon cette expression réguliére
...
def calculsValeursPoids(vecteursE):
	allWeight=np.random.random_sample((len(vecteursE[0][0]),)) # on choisit d'initialiser un seul vecteur de
...
def affectetion(tweetsAClassifier,weights,x,t):
    ...
    for key in tweetsAClassifier.keys(): # pour chaque tweet
        value,tweet,setTweet,vector,vector2,valeurPossibles=tweetsAClassifier[key]# on calcule
        v=vector
        if x>0:
            v=vector2
...
def actifLearnig(dictTw, tweetsEntrainement, classify ,noClassify):
    ...
    print("Éléments classés: "+str(len(classify))+" soit "+str(pourcentClassi)+"%")
    print("Éléments non classés: "+str(len(noClassify)))
    if pourcentClassi<=60:
        vecteursE, dictionnaireModele1, dictionnaireModele2Local=vecteursEntrainement(tweetsEntrainement)
        tweetsAClassifier=vecteursClassication(noClassify,dictionnaireModele1,dictionnaireModele2Local)
        noClassify={}
        poidsMod1,poidsMod2=calculsValeursPoids(vecteursE)
