...
def main():
	...
	for i in range(0,lim):
		...
		nomfichier= "test3.6."+str(i)+".txt" #input("Nommer votre fichier de sortie : ")+".txt" # on nomme le fichier que l'on va créer à partir
		files=["pmilexicon/unigrams-pmilexicon.txt","pmilexicon/bigrams-pmilexicon.txt","pmilexicon/sentimenthashtags.txt", "smiley.txt"]
		sbd=sentimentalBindingDictionnary(files)
		train="training.txt"# notre fichier d'entraînement
		vectors, dictonary = tools(train,sbd) # vectors représente les vecteurs d'entraînement généré des tweets
...
def tools(train,bindingDictionnary): # crée les outils nécessire à l'analyse à partir des données d'entrainment
	...
	for elts in tweets: # pour chauqe tweet:
		...
		vectorLing=linguisticVector(tweet, bindingDictionnary, value)
		vectors.append((vector,vectorLing,values))
	tools=[vectors,dictonary]
	return
...
def linguisticVector(tweet, bindingDictionnary, value):
	setTweet= set(tweet)
	i=0 # on crée les bigrams dans l'ensemble des mot du tweet.
	for bi in tweet:
		if i+1 < len(tweet):
	...
	if inter(setTweet,set(dictSentIndice.keys())):
		for key in setTweet.intersection(dictSentIndice.keys()):
			vector2[dictSentIndice[key]]=dictSentimental[key]
	return vector2
...
def weightsValue(vectors):# weightsValue calcule les différents vecteurs de poids avec une tolérance d'erreur
	...
	#wNeutral=neurone(vectors,newAllWeight, "negative","ling", 0.10)
